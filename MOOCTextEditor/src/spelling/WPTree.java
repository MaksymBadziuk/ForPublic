...
public class WPTree implements WordPath {
	...
	{
		// Add first word as a root of WordPath Tree
	    ...
	    while(!queue.isEmpty() && !pathFound) {
	    	...
	    	// Take first node from a queue
	    	WPTreeNode nextNode = queue.remove(0);
	    	
	    	// Create nearby words
	    	List<String> words = nw.distanceOne(nextNode.getWord(), true);
	    	
	    	// Create nodes for the tree, and add them
	    	for (String string : words) {
	    		...
	    		if (!visited.contains(string)) {
	    			...
	    			visited.add(string);
	    			WPTreeNode newNode = nextNode.addChild(string);
	    			queue.add(newNode);
	    			
	    			// if found target word, exit
	    ...
	    // if Path found
	    if (resultNode != null && pathFound) {
	    	retList = resultNode.buildPathToRoot();
	    }
